...
def _find_inorder_successor(tree, key_node):
    current = key_node
    ...
    # start from the root and keep track of successor until key is found
    while tree is not None:
        if key_node.data < tree.data:
            tree = tree.left
        elif key_node.data > tree.data:
            successor = tree
            tree = tree.right
        else:
            break
...
def get_inorder_successor(tree, key):
    ...
    return _find_inorder_successor(tree, key_node)
