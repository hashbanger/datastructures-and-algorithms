# printing all the nodes that are at k distance from the root using queue method
class Queue:
...
def k_distance_nodes(tree, level):
    ...
    # we will keep a count of the level we have reached
    level_num = 1

    # until the queue is empty
    while not queue.is_empty():

        # count how many elements are in queue at this level (number of nodes)
        level_count = queue.get_size()

        # if we have reached the desired level then
        if level_num == level:

            # dequeue and print all the nodes at that level
            for _ in range(level_count):
                item = queue.dequeue()
                print(item.data, end=" ")
            return

        # otherwise for each node we add its child
        while level_count > 0:
            ...
            # enqueue its children nodes if they exist
            if item.left:
